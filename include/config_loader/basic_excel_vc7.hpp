...
struct LittleEndian
{
	...
	static void Read(const char* buffer, wchar_t& retVal, int pos, int bytes)
	{
		retVal = wchar_t(0);
	...
	static void ReadString(const char* buffer, wchar_t* str, int pos, int bytes)
	{
		for (int i=0; i<bytes; ++i) Read(buffer, str[i], pos+i*SIZEOFWCHAR_T, 0);
	}

	static void Write(char* buffer, wchar_t val, int pos, int bytes)
	{
		if (bytes == 0) bytes = SIZEOFWCHAR_T;
		for (int i=0; i<bytes; ++i)
		{
	...
	static void WriteString(char* buffer, wchar_t* str, int pos, int bytes)
	{
		for (int i=0; i<bytes; ++i) Write(buffer, str[i], pos+i*SIZEOFWCHAR_T, 0);
	}

	static void Read(const vector<char>& buffer, wchar_t& retVal, int pos, int bytes)
	{
		retVal = wchar_t(0);
		if (bytes == 0) bytes = SIZEOFWCHAR_T;
		for (int i=0; i<bytes; ++i)
	...
	static void ReadString(const vector<char>& buffer, wchar_t* str, int pos, int bytes)
	{
		for (int i=0; i<bytes; ++i) Read(buffer, str[i], pos+i*SIZEOFWCHAR_T, 0);
	}

	static void Write(vector<char>& buffer, wchar_t val, int pos, int bytes)
	{
		if (bytes == 0) bytes = SIZEOFWCHAR_T;
		for (int i=0; i<bytes; ++i)
		{
	...
	static void WriteString(vector<char>& buffer, wchar_t* str, int pos, int bytes)
	{
		for (int i=0; i<bytes; ++i) Write(buffer, str[i], pos+i*SIZEOFWCHAR_T, 0);
	}
};
...
public:
	enum {UNDEFINED, INT, DOUBLE, eSTRING, WSTRING};
